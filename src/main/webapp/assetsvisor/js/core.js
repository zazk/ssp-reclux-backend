...
//var rutaCartografia='http://localhost:8084/mimp.gis/assetsvisor/carto/';
...
var repConf=[
    ...
    
    {
      ...
      reportes:[
          ...
          },
          {
             ...
             variables:['NUM_GRUP_EDAD0','NUM_GRUP_EDAD1','NUM_GRUP_EDAD2','NUM_GRUP_EDAD3','NUM_GRUP_EDAD4','NUM_GRUP_EDAD6','NUM_GRUP_EDAD5','NUM_GRUP_EDAD8'] ,
             alias:['< 1','1-5','6-11','12-17','18-M\u00e1s','18-25','26-59','60-M\u00e1s'] 
          }
      ]
    }
    ...
    
    {
      ...
      reportes:[
          ...
          {
             tipo:'r2',                  
             titulo:' EJECUCI\u00F3N PRESUPUESTAL EN EL REGISTRO DE PERSONAS CON DISCAPACIDAD',
             variables:['NUM_INV_EJEC','NUM_INV_PIM'],
             alias:[' Inversi\u00f3n Ejecutada','PIM'] 
          },
          {
    ...
    
    {
      ...
      reportes:[
          ...
          },
          {
             ...
             tipo:'r3',                   
             titulo:' PERSONAS ATENDIDAS POR SEXO EN EL CENTRO DE ATENCION RESIDENCIAL PARA PERSONAS ADULTAS MAYORES',
             variables:['NUM_BEN_EJEC1_H','NUM_BEN_EJEC1_M'],
             alias:['Hombres','Mujeres'] 
          }
      ]
...
lib.createTab = function(id, name, type, callback, tooltip){
   ...
   // tooltip="uff";
    ...
    var $lk = $('<li><a id="tab-'+id+'" title="'+ tooltip +'" href="#tab-content-'+id+'">'+ name +'<i class="led-act '+cl+'"></i></a></li>');
    $("#myTab").append($lk);
...
lib.grid = function(jsn){
     ...
     lib.createTab(jsn.tabId, jsn.tabTitle, jsn.typeserv, function($tb){
        var $dd = $('<div class="dr-table"><div class="header-table"><i class="fa fa-table"></i>&nbsp;  '+jsn.title+' <i class="fa fa-angle-double-down"></i></div><div class="cnt-tb"><a href="#" onclick=exportarTabla("'+jsn.idTabla+'");>Exportar</a><div id="'+jsn.idTabla+'"><table  class="beutif">'+jsn.data.join('')+'</table></div></div></div>'); 
        //var $dd = $('<div class="dr-table"><div class="header-table"><i class="fa fa-table"></i>&nbsp;  '+jsn.title+' <i class="fa fa-angle-double-down"></i></div><div class="cnt-tb"><table class="beutif"><tr><td>'+jsn.idTabla+'</td></tr></table></div></div>'); 
...
$(document).on("ready", function(){
       ...
       Highcharts.setOptions({ colors: [ "#ff5050", "#5b9bd5", "#7f7f7f", "#5BB458"], lang: {
        decimalPoint: ',',
        thousandsSep: ' '
    }});
...
function exportarTabla(idTable){
    window.open('data:application/vnd.ms-excel,' + encodeURIComponent($('#'+idTable).html()));
    //alert("exp:"+ idTable);
}
...
function getCentrosDepa(ubigeo){              
          ...
          if (flgVraem){
            param="q=select CA.COD_CA, CA.REF_CA,CA.DIR_CA,CA.TELEF_CA,SM.SIGLAS1 AS SIGLAS_SERV, SM.TIP_SERV,SM.NOM_SERV, CA.COD_SERV,NOM_CA,COORD_X, COORD_Y from  GEO_CENTRO_ATENCION CA INNER JOIN GEO_SERVICIOS_MIMP SM ON SM.COD_SERV=CA.COD_SERV  where COORD_X is not null and  COORD_Y is not null and  COD_PROV in (0302,0306,0504,0505,0809,0905,0907,1202,1201,1206)";   
          }else{
            param="q=select CA.COD_CA, CA.REF_CA,CA.DIR_CA,CA.TELEF_CA,SM.SIGLAS1 AS SIGLAS_SERV, SM.TIP_SERV,SM.NOM_SERV, CA.COD_SERV,NOM_CA,COORD_X, COORD_Y from  GEO_CENTRO_ATENCION CA INNER JOIN GEO_SERVICIOS_MIMP SM ON SM.COD_SERV=CA.COD_SERV  where COORD_X is not null and  COORD_Y is not null and  COD_DEPAR=" + ubigeo  ;		             
          }
          //alert(param);
	  var h=new Date();	
...
function getCentrosProv(ubigeo){          
          ...
          var param="q=select CA.COD_CA,CA.REF_CA, CA.DIR_CA,CA.TELEF_CA,SM.SIGLAS1 AS SIGLAS_SERV, SM.TIP_SERV, SM.NOM_SERV, CA.COD_SERV,NOM_CA,COORD_X, COORD_Y from  GEO_CENTRO_ATENCION CA INNER JOIN GEO_SERVICIOS_MIMP SM ON SM.COD_SERV=CA.COD_SERV  where COORD_X is not null and  COORD_Y is not null and  COD_PROV=" + ubigeo  ;		          
	  var h=new Date();	
...
function getCentrosDist(ubigeo){          
          ...
          var param="q=select CA.COD_CA,CA.REF_CA,CA.DIR_CA,CA.TELEF_CA,SM.SIGLAS1 AS SIGLAS_SERV, SM.TIP_SERV, SM.NOM_SERV, CA.COD_SERV,NOM_CA,COORD_X, COORD_Y from  GEO_CENTRO_ATENCION CA INNER JOIN GEO_SERVICIOS_MIMP SM ON SM.COD_SERV=CA.COD_SERV  where COORD_X is not null and  COORD_Y is not null and  COD_DIST=" + ubigeo  ;		          
	  var h=new Date();	
...
function reporteNacional(){    
    ...
    var param="q=SELECT S.TIP_SERV,S.NOM_SERV,S.SIGLAS1 AS SIGLAS_SERV,R.COD_SERV,R.ANO,R.PERIODO,E.NOM_ENTIDAD,S.DESCRIPCION";
    param+=varQuery;    
    param+=" FROM GEO_REPORTE_SERVICIOS R  INNER JOIN GEO_ENTIDAD E ON E.COD_ENTIDAD=R.COD_ENTIDAD INNER JOIN GEO_SERVICIOS_MIMP S ON S.COD_SERV=R.COD_SERV   INNER JOIN V_ULTIMOS_REPORTES VUR ON VUR.COD_ENTIDAD=R.COD_ENTIDAD AND VUR.COD_SERV=R.COD_SERV AND VUR.ANO=R.ANO AND VUR.TOP_FECHA=R.FEC_ENVIO GROUP BY S.TIP_SERV,S.NOM_SERV,S.SIGLAS1,R.COD_SERV,R.ANO ,R.PERIODO,E.NOM_ENTIDAD,S.DESCRIPCION ORDER BY R.ANO , R.COD_SERV ASC";
    
   
   /*
    var param="q=SELECT S.TIP_SERV,S.NOM_SERV,S.SIGLAS_SERV,R.COD_SERV,R.ANO,R.PERIODO,E.NOM_ENTIDAD,R.COD_CA,R.NOM_CA";
    param+=varQuery;    
    param+=" FROM GEO_REPORTE_SERVICIOS R  INNER JOIN GEO_ENTIDAD E ON E.COD_ENTIDAD=R.COD_ENTIDAD INNER JOIN GEO_SERVICIOS_MIMP S ON S.COD_SERV=R.COD_SERV   INNER JOIN V_ULTIMOS_REPORTES VUR ON VUR.COD_ENTIDAD=R.COD_ENTIDAD AND VUR.COD_SERV=R.COD_SERV AND VUR.ANO=R.ANO AND VUR.TOP_FECHA=R.FEC_ENVIO GROUP BY S.TIP_SERV,S.NOM_SERV,S.SIGLAS_SERV,R.COD_SERV,R.ANO ,R.PERIODO,E.NOM_ENTIDAD, R.COD_CA, R.NOM_CA ORDER BY R.ANO , R.COD_SERV ASC";
    */
    ...
    var param="q=SELECT S.TIP_SERV, S.NOM_SERV,R.COD_SERV,S.SIGLAS1 AS SIGLAS_SERV,R.DEPART_CA,R.PERIODO,R.ANO ,E.NOM_ENTIDAD,S.DESCRIPCION";
    param+=varQuery;
    //param+=" from GEO_REPORTE_SERVICIOS R   INNER JOIN GEO_SERVICIOS_MIMP S ON S.COD_SERV=R.COD_SERV    INNER JOIN V_ULTIMOS_REPORTES VUR ON VUR.COD_ENTIDAD=R.COD_ENTIDAD AND VUR.COD_SERV=R.COD_SERV AND VUR.ANO=R.ANO AND VUR.TOP_FECHA=R.FEC_ENVIO  GROUP BY S.TIP_SERV,S.NOM_SERV,R.COD_SERV,S.SIGLAS_SERV,R.DEPART_CA,R.PERIODO,R.ANO ORDER BY R.COD_SERV,R.DEPART_CA";
    param+=" from GEO_REPORTE_SERVICIOS R  INNER JOIN GEO_ENTIDAD E ON E.COD_ENTIDAD=R.COD_ENTIDAD  INNER JOIN GEO_SERVICIOS_MIMP S ON S.COD_SERV=R.COD_SERV    WHERE R.ANO = (SELECT MAX(ANO) FROM  V_ULTIMOS_REPORTES)  GROUP BY S.TIP_SERV,S.NOM_SERV,R.COD_SERV,S.SIGLAS1,R.DEPART_CA,R.PERIODO,R.ANO,E.NOM_ENTIDAD,S.DESCRIPCION ORDER BY R.COD_SERV,R.DEPART_CA";
    var h=new Date();	
    // alert(param);    
    var jqxhr2=$.post("http://"+dominioSite+"/mimp.gis/pages/home/info?"+h, param,function(datos) {                                         
        $.each(repConf, function(n, item) {             
...
function reporteDepartamental(ubigeo){
    ...
    if (flgVraem){       
       ...
       param="q=select S.TIP_SERV,S.NOM_SERV,S.SIGLAS1 AS SIGLAS_SERV,R.COD_SERV,R.ANO,R.PERIODO,E.NOM_ENTIDAD,S.DESCRIPCION ";
       param+=varQuery;
       param+=" from GEO_REPORTE_SERVICIOS R INNER JOIN GEO_ENTIDAD E ON E.COD_ENTIDAD=R.COD_ENTIDAD INNER JOIN GEO_SERVICIOS_MIMP S ON S.COD_SERV=R.COD_SERV INNER JOIN V_ULTIMOS_REPORTES VUR ON VUR.COD_ENTIDAD=R.COD_ENTIDAD AND VUR.COD_SERV=R.COD_SERV AND VUR.ANO=R.ANO AND VUR.TOP_FECHA=R.FEC_ENVIO where  R.UBIGEO IN ("+dstVraem+") group by S.TIP_SERV,S.NOM_SERV,S.SIGLAS1,R.COD_SERV,R.ANO,R.PERIODO,E.NOM_ENTIDAD,S.DESCRIPCION ORDER BY R.COD_SERV,R.ANO ASC " ;
    }else{
    ...
    if (flgVraem){
       ...
       param="q=SELECT  S.TIP_SERV,S.NOM_SERV,R.COD_SERV,S.SIGLAS1 AS SIGLAS_SERV,R.PROVIN_CA,R.PERIODO,R.ANO,E.NOM_ENTIDAD,S.DESCRIPCION ";
       param+=varQuery;
       param+=" from GEO_REPORTE_SERVICIOS R INNER JOIN GEO_ENTIDAD E ON E.COD_ENTIDAD=R.COD_ENTIDAD INNER JOIN GEO_SERVICIOS_MIMP S ON S.COD_SERV=R.COD_SERV INNER JOIN V_ULTIMOS_REPORTES VUR ON VUR.COD_ENTIDAD=R.COD_ENTIDAD AND VUR.COD_SERV=R.COD_SERV AND VUR.ANO=R.ANO AND VUR.TOP_FECHA=R.FEC_ENVIO where UBIGEO IN ("+dstVraem+") and R.ANO= (SELECT MAX(ANO) FROM GEO_REPORTE_SERVICIOS )   GROUP BY S.TIP_SERV,S.NOM_SERV, R.COD_SERV,S.SIGLAS1,R.PROVIN_CA,R.PERIODO,R.ANO,E.NOM_ENTIDAD, S.DESCRIPCION ORDER BY R.COD_SERV,R.PROVIN_CA" ;		           
    }else{
...
function reporteProvincial(ubigeo){
 ...
 //alert(ubigeo);
    ...
    if (flgVraem){       
       ...
       param="q=SELECT  S.TIP_SERV,S.NOM_SERV,R.COD_SERV,S.SIGLAS1 AS SIGLAS_SERV,R.DISTR_CA,R.PERIODO,R.ANO,E.NOM_ENTIDAD,S.DESCRIPCION ";
       param+=varQuery;
       param+=" from GEO_REPORTE_SERVICIOS R INNER JOIN GEO_ENTIDAD E ON E.COD_ENTIDAD=R.COD_ENTIDAD INNER JOIN GEO_SERVICIOS_MIMP S ON S.COD_SERV=R.COD_SERV  INNER JOIN V_ULTIMOS_REPORTES VUR ON VUR.COD_ENTIDAD=R.COD_ENTIDAD AND VUR.COD_SERV=R.COD_SERV AND VUR.ANO=R.ANO AND VUR.TOP_FECHA=R.FEC_ENVIO where SUBSTR (R.UBIGEO,0,4)="+ubigeo+" and R.UBIGEO IN ("+dstVraem+")  and R.ANO= (SELECT MAX(ANO) FROM GEO_REPORTE_SERVICIOS )   GROUP BY S.TIP_SERV,S.NOM_SERV,R.COD_SERV,S.SIGLAS1,R.DISTR_CA,R.PERIODO,R.ANO,E.NOM_ENTIDAD,S.DESCRIPCION ORDER BY R.COD_SERV,R.DISTR_CA"; 
    }else{       
...
function reporteDistrital(ubigeo){
    ...
    var param="q=SELECT  S.TIP_SERV,S.NOM_SERV,R.COD_SERV,S.SIGLAS1 AS SIGLAS_SERV,R.DISTR_CA,R.ANO,R.PERIODO,E.NOM_ENTIDAD,S.DESCRIPCION ";
    param+=varQuery;
    param+=" from GEO_REPORTE_SERVICIOS R INNER JOIN GEO_ENTIDAD E ON E.COD_ENTIDAD=R.COD_ENTIDAD INNER JOIN GEO_SERVICIOS_MIMP S ON S.COD_SERV=R.COD_SERV   INNER JOIN V_ULTIMOS_REPORTES VUR ON VUR.COD_ENTIDAD=R.COD_ENTIDAD AND VUR.COD_SERV=R.COD_SERV AND VUR.ANO=R.ANO AND VUR.TOP_FECHA=R.FEC_ENVIO where   SUBSTR(R.UBIGEO,0,6)="+ubigeo+" and R.ANO = (SELECT MAX(ANO) FROM  V_ULTIMOS_REPORTES)  GROUP BY S.TIP_SERV,S.NOM_SERV,R.COD_SERV,S.SIGLAS1,R.DISTR_CA,R.ANO,R.PERIODO,E.NOM_ENTIDAD,S.DESCRIPCION ORDER BY R.COD_SERV,R.DISTR_CA";      
...
function redirReporteHist(servicio, reportes,datos, titulo){
 ...
 $.each(reportes, function(n, item) {              
            switch(item.tipo) { // R1= Historico por años, barra simple  // R2= historico por años, barra doble //  R3    
             case "r1":
                generaReporte_R1 (servicio, item, datos, titulo +  item.titulo);
                break;
             case "r2":
...
function generaReporte_R1 (servicio, varsRep, datos, titulo,titColDesc,fielColDesc){
           ...
           /*
            ...
            //determinar años distintos
            var distinctAniosPeriodos=[];
            $.each(datos, function(x, itemFields) {  
                           var field=itemFields.ANO+"<br>"+capitalizarTexto(itemFields.PERIODO);                                 
                           if (distinctAniosPeriodos.indexOf(field, 0)=== -1 ){                         
                               distinctAniosPeriodos.push(field);                           
                            }
            });   
            ...
            $.each(varsRep.variables, function(a, item) {                   
                   ...
                   dataEjecucionPre.push(arr_);  
            });                        
            dataTblDevengados.push('</tr>','</thead>');
            
                                                
            var flagNew=false;
            $.each(distinctAniosPeriodos, function(j, anio) {                                
                 //categorias.push(anio);
                                
                 $.each(varsRep.variables, function(a, item) {  
                     
                      var sumForVariable=0;
                      var aniox;
                      $.each(datos, function(x, item) {
                          if(datos[x].COD_SERV===servicio && (datos[x].ANO+"<br>"+capitalizarTexto(datos[x].PERIODO))===anio){
                              aniox=anio;
                              ...
                              sumForVariable+=datos[x][varsRep.variables[a]];
                              totalizador+=sumForVariable;
                          }
                      });
                      if (sumForVariable>0){
                          if (categorias.indexOf(aniox, 0)=== -1 ){   
                               //alert (aniox);
                               categorias.push(aniox);                           
                          }
                          dataEjecucion[a].push(sumForVariable);
                      }                       
            ...
            if (flagNew && totalizador>0){                
              ...
              generaBarraDbl2(servicio,labelTab,capitalizarTexto(titulo) ,categorias,dataEjecucion,varsRep,dataPIM,dataDev,tipServ,"Fuente : " + capitalizarTexto(entidad));
            }
   ...
   // alert('rep1');
            ...
            $.each(datos, function(x, item) { 
                 ...
                 if(datos[x].COD_SERV===servicio){
                     ...
                     labelTab=datos[x].SIGLAS_SERV;
                     nomServicio=datos[x].NOM_SERV;
                     descripServ=datos[x].DESCRIPCION;
                     tipServ=datos[x].TIP_SERV;
                     periodo=datos[x].PERIODO;  
                     entidad=datos[x].NOM_ENTIDAD;
            ...
            if (flagNew && totalizador>0){                
              ...
              //alert(sumatorias[1]);
              //Generar Barra grafica                               
              generaBarraDbl2(servicio,labelTab,capitalizarTexto(titulo) ,categorias,dataEjecucion,varsRep,dataPIM,dataDev,tipServ,"Fuente : " + capitalizarTexto(entidad),"Nro. Personas Atendidas",descripServ);
              ...
              var idtabla=servicio + "_" +  Math.floor((Math.random() * 100) + 1);
              generaTabla(servicio,labelTab,capitalizarTexto(titulo),dataTblDevengados,tipServ,idtabla,descripServ);              
...
//Reporte historico tipo barra doble columna   
            ...
            //alert(sumatorias[1]);
            //Generar Barra grafica                               
            generaBarraDbl2(servicio,labelTab,capitalizarTexto(titulo) ,categorias,dataEjecucion,varsRep,dataPIM,dataDev,tipServ,"Fuente : " + capitalizarTexto(entidad), "Nuevos Soles",descripServ);
    ...
    //reporte por sexo
             var totalizador=0;
              ...
              $.each(datos, function(i, item) {  
                 ...
                 if(datos[i].COD_SERV===servicio){
                   ...
                   nomTab=datos[i].COD_SERV;
                   nomServicio=datos[i].NOM_SERV;
                   descripServ=datos[i].DESCRIPCION;
                   tipServ=datos[i].TIP_SERV;
                   periodo=datos[i].PERIODO;
                   ano=datos[i].ANO;
           ...
           if (flagNew && totalizador>0){
              ...
              $.each(varsRep.variables, function(a, item) {
               ...
               var idtabla=nomTab + "__" +  Math.floor((Math.random() * 100) + 1);
               generaPie(nomTab,tituloIn,capitalizarTexto(titulo)+ "<br>"+ ano + "(" + capitalizarTexto(periodo) +")"  ,dataPie,tipServ, "Fuente : "+ capitalizarTexto(entidad), descripServ);                                            
               generaTabla(nomTab,tituloIn,capitalizarTexto(titulo),data,tipServ,idtabla,descripServ);  
...
function generaReporte_R4 (servicio, varsRep, datos, titulo,titColDesc,fielColDesc){
 ...
 var totalizador=0;
             ...
             if (flagNew && totalizador>0){
              ...
              var idtabla=nomTab + "___" +  Math.floor((Math.random() * 100) + 1);                                                              
              generaBarra(nomTab,tituloIn,capitalizarTexto(titulo)+ "<br>"+ano + "(" + capitalizarTexto(periodo)+")",dataBar,300,tipServ,"Fuente : "+ capitalizarTexto(entidad), descripServ);              
              generaTabla(nomTab,tituloIn,capitalizarTexto(titulo),dataGrupoEdad,tipServ,idtabla,descripServ);
...
function generaBarra(tabId,tabTitle,titulo, datos,alto,tipSrv,subtitle, toolTip){
    //alert(tipSrv);
    lib.graph({'tabId':tabId,'tabTitle':tabTitle,'toolTip':toolTip,
       typeserv:tipSrv,
        chart: {
            type: 'column'
        },
...
function generaBarra_2(tabId,tabTitle,titulo, datos,alto,tipSrv,toolTip){
    lib.graph({'tabId':tabId,'tabTitle':tabTitle,'toolTip':toolTip,
        typeserv:tipSrv,
        chart: {
            type: 'column',
...
function generaBarraDbl2(tabId,tabTitle,titulo, categorias,dataEjecucion,objTitulo,dataPIM,dataDev,tipSrv, subtitle,txtDescY,toolTip){
...
function generaBarraDbl(tabId,tabTitle,titulo, categorias,dataPIM,dataDev,tipSrv,toolTip){
...
function generaPie(tabId,tabTitle,titulo,data,tipSrv,subtitle,toolTip ){
...
function generaTabla(tabId,tabTitle,titulo,datos,tipSrv,idTabla,toolTip){  
    //'tabId':'car','tabTitle':'Titulo de CAR',
    lib.grid({'tabId':tabId,'tabTitle':tabTitle,'title':titulo, 'data': datos , 'typeserv':tipSrv,'idTabla':idTabla,'toolTip':toolTip});
}
