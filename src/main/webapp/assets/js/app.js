...
$(document).ready(function() {
    ...
    $('.dd-child').change(function(e){
        $this = $(this);
        var sql = "select s.cod_serv,s.NOM_SERV from geo_centro_atencion g inner join "+
                "GEO_SERVICIOS_MIMP s on s.cod_serv = g.cod_serv where cod_depar = '"+ $this.val() +"' "+
                "GROUP BY s.NOM_SERV, s.COD_SERV";
        //console.log("GONE!!!" + $this.val() );
        $child = $( $this.data('child'));
        //console.log("My Child!!! " + $this.data('child') + " Element: " + $child.attr("id") );
        //console.log("SQL: " + sql ); 
        
        $.post(URL,{q:sql}, function(r){
            var options = "";
            for(var t = r.length, i=0 ; i<t ; i++){
                var obj = r[i];
                //console.log("Obj",obj);
                options += "<option value='"+ obj.COD_SERV +"'>"+ obj.NOM_SERV+"</option>";
            }
            $child.html(options);
        });
